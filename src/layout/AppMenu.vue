...
console.log(permissionStore.sidebarRouters, 'sidebarRouters');
const loop = (data = [], callback) => {
  data.forEach((datum) => {
    callback(datum);
    if (datum.children?.length) {
      loop(datum.children, callback);
    }
  });
};
const menuList = computed(() => {
  const dataList = cloneDeep(permissionStore.sidebarRouters);
  loop(dataList, (item) => {
    item.visible = !item.hidden;
    item.label = item.meta?.title || '默认';
    item.to = item.path;
